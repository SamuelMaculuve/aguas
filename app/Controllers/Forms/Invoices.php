...
class Invoices extends BaseController
{
    ...
    public function add($data): void
    {
        ...
        if ($consumption <= $_SESSION['config']['base_volume']) {
            $amount = floatval($_SESSION['config']['base_price']);
        } else {
        ...
        switch ($_SESSION['config']['expiry_mode']) {
            ...
            case 2:
                $today = new DateTime("now");
                $expiry_date = $today->add(new DateInterval("P1M"));
                $expiry_date = $expiry_date->format("{$_SESSION['config']['expiry_date']}-m-Y");
                break;
        }
